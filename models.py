...
class FrameAggregation2D(nn.Module):
    ...
    def __init__(self, num_classes):
        super().__init__()
        self.backbone = Simple2DCNN(num_classes=num_classes)
...
class LateFusion2D(nn.Module):
    ...
    def __init__(self, num_classes):
        super().__init__()
        self.rgb_model = Simple2DCNN(num_classes=num_classes)
        self.flow_model = Simple2DCNN(num_classes=num_classes)

        self.fc = nn.Linear(2 * 256, num_classes)

    def forward(self, rgb, flow):
        B, T, C, H, W = rgb.shape
        rgb_feats = self.rgb_model(rgb.view(B*T, C, H , W).view(B, T, -1)).mean(1)
        flow_feats = self.flow_model(flow.view(B*T, C, H , W).view(B, T, -1)).mean(1)
